...
- **#66–#67 · Stage 2 存储与大库收口**（进行中）。用户于 2026-09-23 验收了上一轮可见功能。本轮代码把本地库启动限制为 128 首、资料库歌曲页和 Spotlight 改为分页或目标查询，播放队列按需续页且历史有界，显式元数据修复与指纹孤儿清理也逐页读取；补了歌单写失败回滚、旧数据迁移重试与跨存储对账测试。下一步处理专辑/艺人摘要整表常驻，再在固定磁盘数据集上测应用冷启动与 RSS；未拿到这两项证据前 #67 不完成。
