...
| Preview | 44 个直接 View/Representable 有同文件 Preview，使用隔离数据与依赖；包含空、有内容、混合队列、检查器、曲库表格、Radio Hero 与卡片、自定义电台弹窗及窄布局。门禁识别后置协议、extension、枚举，过滤注释和字符串；间接协议仍需代码审查。 |
...
- 门禁：架构（0 违规）、Preview（44 个全部覆盖）、diff 检查通过；入口链接须在修改后继续检查。源码检查不能替代渲染或运行时验收。
