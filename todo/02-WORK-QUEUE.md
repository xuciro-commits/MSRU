...
- **#66–#67 · Stage 2 存储与大库收口**（进行中）。用户于 2026-09-23 验收了上一轮可见功能。本轮代码把本地库启动限制为 128 首、资料库歌曲页和 Spotlight 改为分页或目标查询，补了歌单写失败回滚、旧数据迁移重试与跨存储对账测试。下一步处理专辑/艺人摘要整表常驻、分页播放队列和显式修复操作的整表读取，再在固定磁盘数据集上测应用冷启动与 RSS；未拿到这两项证据前 #67 不完成。
...
| **#67 · 进行中** | **50,000 首曲库性能与流式展示**：本地歌曲页、常用目标查询及 Spotlight 已分页；代码基准（50,000 首/1,000 专辑/100 艺人，测试进程内 SQLite）本地库载入 128 首与摘要约 86.6ms，整表读取约 454ms。下一步对专辑/艺人摘要、播放队列和显式修复操作分页，并测固定磁盘数据集的应用冷启动和 RSS。依据[大曲库架构](../Docs/Architecture/LargeLibraryAndIdentityArchitecture.md)。 | 固定设备、数据集、冷启动起止点和当前内存基线，再验证路线图的 <100ms、内存下降 70% 目标；保留可复现基准结果，达不到则记录瓶颈并继续优化，不凭空宣称达标。用户验收滚动体验。 |
