...
以中位数计，整进程 RSS 由约 240MiB 降至 177MiB，下降约 **26%**；各自扣除同版本空库进程基线后，曲库带来的额外 RSS 由约 78MiB 降至约 19MiB，下降约 **76%**。这是两种不同的分母，不能把后者表述成“总内存下降 70%”。当前版与空库版启动时间接近，但进程从执行到数据就绪仍约 0.7 秒，**不符合**路线图原文的全 App `<100ms`。启动基线主要在 App/系统装配范围，不能由曲库分页独自消除。用户于 2026-09-24 接受该实测结果，确认 #67/#68 收尾并转入 Stage 3；这是范围验收，不表示原全进程指标被测得通过。
