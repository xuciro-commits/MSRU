...
**Not:** an Apple UI framework (that is one client layer, see [AppleClient.md](AppleClient.md)); the intersection of Music and Hotel; a generic business-object or ERP schema; a configuration language that replaces domain code.
...
| K9 Work ownership | Long-running work has an owner, cancellation, stale-result invalidation and resumable checkpoints; closing an owner never silently reverts committed decisions | Server workflows and client tasks cannot share these semantics | H (client side implemented in `FeatureHost`) |
...
**Current coverage.** K1 Identity, K2 Fact kinds with K3 Provenance, K4 Change record (`v1alpha1`). K4 C1 now also requires `authority`, the provenance of a decision (breaking, alpha).
