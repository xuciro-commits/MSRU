...
**Consequences.** AppFoundation may not depend on domain packages or storage engines, may not re-export modules, and may not expose domain vocabulary (enforced by `Scripts/verify-architecture.py`). Kernel concepts enter only through the promotion rules in Platform.md §4.
