...
| `Packages/MusicDomain` — target `MusicLibrary` | Library, persistence (GRDB/SQLite + migrations), identity, import pipeline, catalogue providers, library health scan, queries, radio, lyrics fetching |
...
- **Library Cleanup finds, then fixes.** `LibraryHealthScanner` reads the index in keyset pages (never files) and reports duplicate candidates (normalized artist + title, duration within 6 s), incomplete info (placeholder title, unknown artist, missing album; user corrections count) and missing artwork. Only candidates are then read from disk by the duplicate classifier, and only affected tracks go through Get Info, in batches of 20. A fix is measured by rescanning. Only local files are scanned.
