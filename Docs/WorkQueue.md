...
| **76 · next** | Music domain packages: move `MSRU/Music` (and MediaLibrary/SubsonicKit) into domain packages so the app target only composes, hosts features and adapts platforms; unify the two source representations; remove the transitional link-only dependencies from MusicDomain. User-visible behaviour unchanged | App assembles from packages; `verify.sh` passes; owner spot-checks main flows |
