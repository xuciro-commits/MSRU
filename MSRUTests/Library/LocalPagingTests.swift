...
struct LocalPagingTests {
    ...
    func fiftyThousandRows() async throws {
        ...
        let queueSource = try #require(pager.makePlaybackPageSource())
        let continued = try await queueSource.nextPage()
        #expect(continued.count == 128)
        #expect(continued.first?.title == "Song 00256")
        #expect(pager.tracks.count == 256)
