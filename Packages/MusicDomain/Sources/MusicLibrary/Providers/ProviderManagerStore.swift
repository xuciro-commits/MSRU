...
@MainActor
@Observable
public final class ProviderManagerStore {

    private static let defaultsKey = "MSRU.ProviderManager.v1"

    public private(set) var providers: [ManagedProvider] = []
    public private(set) var testingProviderIDs: Set<UUID> = []
    ...
    public var orderedProviders: [ManagedProvider] {
        providers.sorted {
    ...
    public var playbackProviders: [ManagedProvider] {
        orderedProviders.filter { $0.capabilities.contains(.playback) }
    }

    public func provider(id: UUID) -> ManagedProvider? {
        providers.first { $0.id == id }
    }

    public func isTesting(id: UUID) -> Bool {
        testingProviderIDs.contains(id)
    }

    @discardableResult
    public func addRemoteProvider(
        name: String,
        endpoint: String,
    ...
    public func setEnabled(_ isEnabled: Bool, id: UUID) {
        guard let index = providers.firstIndex(where: { $0.id == id }) else { return }
        providers[index].isEnabled = isEnabled
        persist()
    }

    public func setPriority(_ value: Int, id: UUID) {
        update(id: id) { $0.priority = max(0, value) }
    }

    public func setName(_ value: String, id: UUID) {
        update(id: id) {
    ...
    public func setEndpoint(_ value: String, id: UUID) {
        update(id: id) {
            guard $0.isCustom else { return }
    ...
    public func moveProviders(fromOffsets offsets: IndexSet, toOffset destination: Int) {
        providers.reorder(fromOffsets: offsets, toOffset: destination)
    ...
    public nonisolated static func validHTTPURL(_ value: String) -> URL? {
