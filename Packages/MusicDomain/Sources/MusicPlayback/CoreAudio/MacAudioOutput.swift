...
public struct MacAudioDevice: Equatable, Identifiable {
    public let id: AudioDeviceID
    public let uid: String
    public let name: String
    public let nominalRate: Double
    public let availableRates: [AudioValueRange]
    ...
    public func supports(_ rate: Double) -> Bool {
    ...
    nonisolated public init(
        ...
        name: String,
        nominalRate: Double,
        availableRates: [AudioValueRange]
    ) {
        self.id = id
        self.uid = uid
        self.name = name
        self.nominalRate = nominalRate
        self.availableRates = availableRates
    }
}

public struct MacAudioOutputRoute: Equatable {
    ...
    nonisolated public init(
        ...
        self.hardwareRate = hardwareRate
...
@MainActor
public protocol MacAudioHardware {
...
public enum MacAudioOutputError: LocalizedError {
...
@MainActor
public final class CoreAudioHardware: MacAudioHardware {
    public func outputDevices() throws -> [MacAudioDevice] {
    ...
    public func defaultOutputID() throws -> AudioDeviceID {
        try scalar(AudioObjectID(kAudioObjectSystemObject), selector: kAudioHardwarePropertyDefaultOutputDevice)
    }

    public func setNominalRate(_ rate: Double, deviceID: AudioDeviceID) throws {
        var value = rate
    ...
    public func hogOwner(deviceID: AudioDeviceID) throws -> pid_t {
        try scalar(deviceID, selector: kAudioDevicePropertyHogMode)
    }

    public func toggleHog(deviceID: AudioDeviceID) throws -> pid_t {
        var owner: pid_t = -1
...
public final class MacAudioOutputController {
    ...
    public private(set) var devices: [MacAudioDevice] = []
    public private(set) var selectedUID: String?
    public private(set) var route: MacAudioOutputRoute?
    public private(set) var engineRate: Double?
    public private(set) var errorMessage: String?
    public private(set) var exclusiveRequested: Bool
    public var onSelectedDeviceLost: (() -> Void)?
    public var onDefaultDeviceChanged: (() -> Void)?

    public init(hardware: any MacAudioHardware = CoreAudioHardware(), preferences: UserDefaults = .standard, observe: Bool = true) {
    ...
    public var displayName: String { route?.deviceName ?? devices.first(where: { $0.uid == selectedUID })?.name ?? "System Default" }
    ...
    public func setExclusive(_ enabled: Bool) {
    ...
    public func prepare(inputRate: Double?) throws -> MacAudioOutputRoute {
        refreshDevices()
    ...
    public func updateEngineRate(_ rate: Double?) {
        engineRate = rate
    }

    public func releaseHardware() {
...
extension AVAudioEngine {
    public func routeToMacOutput(deviceID id: AudioDeviceID?) throws {
        if let id {
