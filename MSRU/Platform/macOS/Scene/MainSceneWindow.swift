...
@MainActor
final class MainSceneWindow: NSObject, MacSceneWindow, NSWindowDelegate {

    // MARK: - Identity
    let sceneID: SceneID

    // MARK: - Scene
    private let scene: SceneModel

    // MARK: - Window Composition
    private let composition: MSRUMacWindowComposition

    // MARK: - Lifecycle Callbacks
    private let onSnapshotChange: @MainActor (SceneRestorationSnapshot) -> Void
    private let onSceneClosed: @MainActor (SceneID) -> Void
    ...
    init(
        ...
        self.onSceneClosed = onSceneClosed
        self.composition = MSRUMacWindowComposition(
            scene: scene,
        ...
        composition.windowController.window?.delegate = self
    ...
    // MARK: - Window State
    var isActive: Bool {
        composition.windowController.window?.isKeyWindow ?? false
    }

    // MARK: - Activation
    func activate() {
        composition.windowController.showWindow(nil)
        composition.windowController.window?.makeKeyAndOrderFront(nil)
    }

    // MARK: - Restoration
    func restorationSnapshot() -> SceneRestorationSnapshot {
        scene.restorationSnapshot()
    }
    ...
    private func observeRestorationState() {
        ...
        withObservationTracking {
            _ = scene.navigation.section
            _ = scene.isQueuePresented
        } onChange: { [weak self] in
            Task { @MainActor [weak self] in
                guard let self, !self.scene.isClosed else { return }
                onSnapshotChange(scene.restorationSnapshot())
                observeRestorationState()
            }
        }
    }
...
@MainActor
final class MainSceneWindowFactory: MacSceneWindowFactory {
